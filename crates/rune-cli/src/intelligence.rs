...
impl IntelligentFileAnalyzer {
    ...
    pub fn analyze_file(&mut self, path: &str) -> Result<FileAnalysis, std::io::Error> {
        ...
        let compression_ratio = self.estimate_compression(&file_type, file_size);
        let should_compress = compression_ratio > 0.3;
        let suggested_lfs = file_size > 10_000_000; // >10MB
        let security_risk = self.analyze_security_risk(path, &file_type);
        let performance_impact = self.analyze_performance_impact(file_size, &file_type);
        let code_quality = self.analyze_code_quality(path, &file_type);
        let dependencies = self.analyze_dependencies(path, &file_type);
        let suggestions = self.generate_suggestions(path, &file_type, &security_risk, &performance_impact);
    ...
    fn display_analysis_notifications(&self, path: &str, analysis: &FileAnalysis) {
        match self.config.notification_level {
            ...
            NotificationLevel::Errors => {
                if matches!(analysis.security_risk.level, RiskLevel::High | RiskLevel::Critical) {
