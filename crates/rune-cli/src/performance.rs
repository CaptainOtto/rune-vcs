...
impl PerformanceEngine {
    ...
    pub fn predictive_cache(&self, _repo_path: &str) -> Result<(), std::io::Error> {
        ...
        let common_operations = vec!["status", "log", "diff", "branch"];
    ...
    pub fn show_stats(&self) {
        ...
        println!(
            "  📊 Cache hits: {}",
            self.metrics.cache_hits.to_string().green()
        );
        println!(
            "  📊 Cache misses: {}",
            self.metrics.cache_misses.to_string().yellow()
        );
        println!(
            "  📊 Operations cached: {}",
            self.metrics.operations_cached.to_string().blue()
        );
