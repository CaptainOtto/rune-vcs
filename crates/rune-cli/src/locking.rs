...
#[derive(Debug, Clone)]
pub enum MediaType {
    Image, Video, Audio, Model3D, Font,
}
...
impl LockManager {
    ...
    pub fn analyze_files(&self, file_paths: &[String]) -> HashMap<String, FileInfo> {
        ...
        for path in file_paths {
            ...
            results.insert(path.clone(), FileInfo {
                file_type,
            });
        }
    ...
    pub fn get_lock_status(&self, _repo_path: &str, file_paths: &[String]) -> HashMap<String, LockStatus> {
    ...
    pub fn detect_file_type(&self, file_path: &str) -> DetectedFileType {
        let path = Path::new(file_path);
        ...
        match extension.as_str() {
            ...
            "mp3" | "wav" | "ogg" | "flac" | "aac" => {
                DetectedFileType::Media(MediaType::Audio)
            },
            "ttf" | "otf" | "woff" | "woff2" => {
                DetectedFileType::Media(MediaType::Font)
            },
    ...
    fn matches_pattern(&self, file_path: &str, pattern: &str) -> bool {
        ...
        if pattern.starts_with("*.") {
            let extension = &pattern[2..]; // Remove "*."
            file_name.ends_with(extension)
        } else if pattern.contains('*') {
            // For patterns like "node_modules/**", check if path contains the prefix
            let pattern_prefix = pattern.replace("/**", "").replace("*", "");
            file_path.contains(&pattern_prefix)
        } else {
            file_name == pattern
        }
    }
